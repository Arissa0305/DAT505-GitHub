...
function init() {
  ...
  // modify UVs to accommodate MatCap texture
	...
	for ( i = 0; i < faceVertexUvs.length; i ++ ) {
		...
		var cubesscale = (Math.random()* -1);
		mesh = new THREE.Mesh( geometry, material );
				mesh.position.x = Math.random() * 300 - 100;
				mesh.position.y = Math.random() * 200 - 50;
...
function render() {
  ...
  cubes[i].rotation.x = mouseY/window.innerHeight*2;//鼠标 控制旋转速度
...
function onWindowResize() {
  windowHalfX = window.innerWidth / 2;//window的大小
